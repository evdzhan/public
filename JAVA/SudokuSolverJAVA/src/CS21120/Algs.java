			...
			for (int o = 0; o < 9; o++) {
				...
				if (the_sudoku.containsKey(key)) {
					if (the_sudoku.get(key).length() == 1) {
						...
						unstableSudoku[i][o] = Integer.parseInt(the_sudoku.get(key));
				...
				for (int i = c; i < c + 3; i++) {
					for (int o = a; o < a + 3; o++) {
						if (i != row) {
							...
							if (the_sudoku.containsKey(key)) {
								if (the_sudoku.get(key).contains(counted)) {
									String replacedOne = the_sudoku.get(key).replaceAll(
											counted, "");
							...
							for (int m = 0; m < 9; m++) {
								if (m != i && m != row) {
									...
									if (the_sudoku.containsKey(key1)) {
										if (the_sudoku.get(key1).contains(pair1)) {
											String replacedOne = the_sudoku.get(key1)
													.replace(pair1, "");
											the_sudoku.put(key1, replacedOne);
											this.lockSingleCandidate();
										}
										if (the_sudoku.containsKey(key1)) {
											if (the_sudoku.get(key1).contains(pair2)) {
