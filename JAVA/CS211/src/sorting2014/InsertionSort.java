...
public class InsertionSort implements Sorter {
	...
	/*
	 * @SuppressWarnings("unchecked") public void insertionSort(Comparable[]
	 * items) {
	 * 
	 * for ( int i = 1 ; i < items.length ; ++i) {
	 * 
	 * Comparable temp = items[i]; int j ; int k = 0 ; for(j = i - 1; j >= 0;
	 * --j) {
	 * 
	 * if(items[j].compareTo(temp) < 0) { break; } k++; items[j+1] = items[j]; }
	 * items[j+1] = temp ; System.out.println("After step : "+i+
	 * ", num of swaps are "+ k+ " and new index is "+ (j+1)); } }
	 */
}
