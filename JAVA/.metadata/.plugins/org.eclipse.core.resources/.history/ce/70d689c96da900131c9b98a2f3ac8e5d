...
public class ShellSort implements Sorter {
	 ...
	 * when the sorting essentially becomes a insertion sort.
	 * 
	...
	private void shellSort(Comparable[] items) {
		...
		for (int gap = items.length / 2 ; gap > 0; ) { //initial gap 1/2 size of the array
			...
			gap = gap == 2 ? 1: (int) ( gap / 2.25 ); // decrement the gap by some factor 
			                // eventually this will become 1, thus becoming a insertion sort
		}
	}
